...
import view.ManHinhNguoiQuanLy;
...
public class Tour_DAO implements ITour{
	...
	public boolean themTour(Tour tour) {
		...
		try {
			String sql = "insert into Tour values(?,?,?,?,?,?,(SELECT * FROM OPENROWSET(BULK N'"+ManHinhNguoiQuanLy.url+"', SINGLE_BLOB) as img),?,?,?,getdate(),?)";
	...
	public boolean suaTour(String id, Tour newTour) {
		...
		PreparedStatement statement = null;
		try {
			String sql = "update Tour set tenTour=?, tgKhoiHanh=?, soNgay=?, soVeConlai=?, gia=?,hinhAnh=(SELECT * FROM OPENROWSET(BULK N'" +ManHinhNguoiQuanLy.url+"', SINGLE_BLOB) as img), tgTapTrung=?, diemDi=?, diemDen=?, tgCapNhat=getdate(), maHDV=? where maTour = ?";
			statement = con.prepareStatement(sql);
	...
	public ArrayList<Tour> timKiem(String noiDungTimKiem) {
		...
		PreparedStatement statement = null;
		try {
			String sql = "Select * from Tour t join HuongDanVien h on t.maHDV=h.maHDV where maTour = ? or tenTour = ? or h.maHDV = ?";// and convert(date,tgKhoiHanh) = ?
			statement = con.prepareStatement(sql);
			statement.setNString(1, noiDungTimKiem);
			statement.setNString(2, noiDungTimKiem);
			statement.setNString(3, noiDungTimKiem);
			//statement.setString(3, ngayDi);
			//System.out.println(diemDi+ diemDen+ ngayDi);
			ResultSet rs = statement.executeQuery();
			while (rs.next()) {
