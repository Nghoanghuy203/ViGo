...
public class ManHinhNguoiQuanLy extends JFrame {
...
//			}
//		});
		new ManHinhNguoiQuanLy().setVisible(true);
	}

	/**
	 * Create the frame.
	 */
	public ManHinhNguoiQuanLy() {
		doFindEvent =false;
		danhMuc=1;
		try {
		...
		btnTroVe.setIcon(new ImageIcon(ManHinhNguoiQuanLy.class.getResource("/images/back.png")));
		...
		btnTim.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				//String T_Header[] =  "Mã Tour, Tên Tour,TG TTrung, TG KhHành,Số Ngày, Số Vé Còn,Giá,Điểm Đi,Điểm Đến,Mã HDV".split(",");
				///String KH_Header[] =  "Số Khách Hàng,Họ Tên,Ngày Sinh,SĐT,Giới Tính,TK Email,Mật Khẩu".split(",");
				String noiDungTimKiem = txtTim.getText().trim();
				if(noiDungTimKiem.equals("")) System.out.println(danhMuc); 
				else {
					doFindEvent = true;
	...
	private void GUI_Tour() {
		...
		CustomButton btnLoc = new CustomButton((String) null);
		btnLoc.setRadius(20);
		btnLoc.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
			}
		});
		btnLoc.setForeground(ChonMau.blue_4B70F5);
		btnLoc.setOver(true);
		btnLoc.setText("LỌC");
		btnLoc.setBounds(680, 18, 55, 26);
		search.add(btnLoc);
		...
		ArrayList<Tour> tourTim = null;
		if(doFindEvent) {
			System.out.println("b");
			String noiDungTimKiem = txtTim.getText().trim();
			tourTim = tour_BUS.timKiem(noiDungTimKiem);
			if (tourTim.size()==0) {
				SomeStaticMethod.showDialog(JOptionPane.WARNING_MESSAGE, "Không tìm thấy kết quả nào!");
				for (Tour tour : dsTour) 
					modelTour.addRow(tour.toString().split(";"));
			}
			else {
				//customTable.xoaTable();
				for (Tour tour : tourTim) {
					modelTour.addRow(tour.toString().split(";"));
				}
			}
		}else {
		...
		btnLoc.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String ngayDi = null;
				try {
					ngayDi = sdf.format(datePicker.getModel().getValue());
				} catch (Exception e2) {
					// TODO: handle exception
					ngayDi ="";
				}
				if(tour_BUS.timKiem(noiDungTimKiem, cboDiemDi.getSelectedItem().toString(), cboDiemDen.getSelectedItem().toString(),ngayDi).size()!=0) {
					modelTour.getDataVector().removeAllElements();
				for (Tour tour : tour_BUS.timKiem(noiDungTimKiem, cboDiemDi.getSelectedItem().toString(), cboDiemDen.getSelectedItem().toString(),ngayDi)) {
					System.out.println(cboDiemDi.getSelectedItem().toString()+" "+ cboDiemDen.getSelectedItem().toString());
					modelTour.addRow(tour.toString().split(";"));
				}
				} else {
					SomeStaticMethod.showDialog(JOptionPane.WARNING_MESSAGE, "Lọc không ra!");
				}
			}
		});
		...
		T_btnThem.setIcon(new ImageIcon(ManHinhNguoiQuanLy.class.getResource("/images/them.png")));
		...
		T_btnXoa.setIcon(new ImageIcon(ManHinhNguoiQuanLy.class.getResource("/images/xoa.png")));
...
//					LocalDate ngayKhoiHanh = LocalDate.of(nam, thang, ngay);
		...
		T_btnSua.setIcon(new ImageIcon(ManHinhNguoiQuanLy.class.getResource("/images/sua.png")));
		...
		T_btnXoaTrang.setIcon(new ImageIcon(ManHinhNguoiQuanLy.class.getResource("/images/pen.png")));
		...
		try {
			h = ImageIO.read(ManHinhNguoiQuanLy.class.getResource("/images/nullimg.png"));
		} catch (IOException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}
...
//		String url;
		...
		T_btnThem.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (validDataTour()) {
					...
					if (url.equals("")) {
						SomeStaticMethod.showDialog(JOptionPane.WARNING_MESSAGE, "Vui lòng chọn ảnh!");
						return;
					}
					File fimg = new File(url);
					BufferedImage img=null;
	...
	private void GUI_KhachHang() {
		...
		customTable.setBounds(10, 90, 761, 387);
		ArrayList<KhachHang> khachHangTim = null;
		if(doFindEvent) {
			System.out.println("b");
			String noiDungTimKiem = txtTim.getText().trim();
			khachHangTim = khachHang_BUS.timKiem(noiDungTimKiem);
			if (khachHangTim.size()==0) {
				SomeStaticMethod.showDialog(JOptionPane.WARNING_MESSAGE, "Không tìm thấy kết quả nào!");
				...
				//customTable.xoaTable();
				for (KhachHang khachHang : khachHangTim) {
					modelKhachHang.addRow(khachHang.toString().split(";"));
				}
		...
		KH_btnThem.setIcon(new ImageIcon(ManHinhNguoiQuanLy.class.getResource("/images/them.png")));
		...
		KH_btnXoa.setIcon(new ImageIcon(ManHinhNguoiQuanLy.class.getResource("/images/xoa.png")));
		...
		KH_btnSua.setIcon(new ImageIcon(ManHinhNguoiQuanLy.class.getResource("/images/sua.png")));
		...
		KH_btnXoaTrang.setIcon(new ImageIcon(ManHinhNguoiQuanLy.class.getResource("/images/pen.png")));
	...
	private void GUI_DonDat() {
		...
		CustomButton btnLoc = new CustomButton((String) null);
		btnLoc.setFont(new Font("Arial", Font.BOLD, 11));
		btnLoc.setHightlight(new Color(255, 255, 255));
		btnLoc.setRadius(20);
		btnLoc.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
			}
		...
		soVeCaoNhat = 10;
		soVeThapNhat = 1;
		tongGiaCaoNhat = 1000000.0;
		tongGiaThapNhat = 100000.0;
		JSpinner spinnerSoVeMax = new JSpinner(new SpinnerNumberModel(10, soVeThapNhat,100, 1));
		JSpinner spinnerSoVeMin = new JSpinner(new SpinnerNumberModel(1, 1, soVeCaoNhat, 1));
		...
		spinnerSoVeMin.addChangeListener((ChangeListener) new ChangeListener() {
			...
			public void stateChanged(ChangeEvent e) {
				...
				spinnerSoVeMax.setModel(new SpinnerNumberModel(soVeCaoNhat,  soVeThapNhat,100, 1));
				//capNhatTable();
			}
		});
		...
		spinnerSoVeMax.addChangeListener((ChangeListener) new ChangeListener() {
			@Override
			public void stateChanged(ChangeEvent e) {
				// TODO Auto-generated method stub
				soVeCaoNhat = (int) spinnerSoVeMax.getValue();
				spinnerSoVeMin.setModel(new SpinnerNumberModel(soVeThapNhat, 1, soVeCaoNhat, 1));
				//capNhatTable();
			}
		});
		...
		JSpinner spinnerTGMin = new JSpinner(new SpinnerNumberModel(100000.0, 0.0, tongGiaCaoNhat, 100000.0));
		JSpinner spinnerTGMax = new JSpinner(new SpinnerNumberModel(1000000.0, tongGiaThapNhat, 10000000000.0, 100000.0));
		...
		spinnerTGMax.addChangeListener((ChangeListener) new ChangeListener() {
			@Override
			public void stateChanged(ChangeEvent e) {
				// TODO Auto-generated method stub
				System.out.println("tang");
				tongGiaCaoNhat = (double) spinnerTGMax.getValue();
				spinnerTGMin.setModel(new SpinnerNumberModel(tongGiaThapNhat, 0.0, tongGiaCaoNhat, 100000.0));
			}
		});
		spinnerTGMin.addChangeListener((ChangeListener) new ChangeListener() {
			@Override
			public void stateChanged(ChangeEvent e) {
				// TODO Auto-generated method stub
				System.out.println("tang");
				tongGiaThapNhat = (double) spinnerTGMin.getValue(); 
				spinnerTGMax.setModel(new SpinnerNumberModel(tongGiaCaoNhat, tongGiaThapNhat, 10000000000.0, 100000.0));
			}
		});
		...
		customTable.setBounds(10, 90, 761, 387);
		ArrayList<DonDatTour> donDatTourTim = null;
		ArrayList<DonDatTour> dsD = donDatTour_BUS.getDSDonDatTour();
		if(doFindEvent) {
			String noiDungTimKiem = txtTim.getText().trim();
			donDatTourTim = donDatTour_BUS.timKiem(noiDungTimKiem);
			if (donDatTourTim.size()==0) {
				SomeStaticMethod.showDialog(JOptionPane.WARNING_MESSAGE, "Không tìm thấy kết quả nào!");
				for (DonDatTour donDatTour : dsD) modelDonDat.addRow(donDatTour.toString().split(";"));
			}
			else {
				//customTable.xoaTable();
				for (DonDatTour donDatTour : donDatTourTim) {
					modelDonDat.addRow(donDatTour.toString().split(";"));
					//System.out.println(donDatTour.toString());
				}
			}
		}else {
			for (DonDatTour donDatTour : dsD) {
			modelDonDat.addRow(donDatTour.toString().split(";"));
		}
		}
		btnLoc.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String ngayDat = null;
				try {
					ngayDat = sdf.format(datePicker.getModel().getValue());
				} catch (Exception e2) {
					// TODO: handle exception
					ngayDat ="";
				}
				//modelDonDat.getDataVector().removeAllElements();
				if (donDatTour_BUS.timKiem(noiDungTimKiem,soVeThapNhat,soVeCaoNhat,tongGiaThapNhat,tongGiaCaoNhat,ngayDat).size()!=0) {
					modelDonDat.getDataVector().removeAllElements();
					for (DonDatTour donDatTour : donDatTour_BUS.timKiem(noiDungTimKiem,soVeThapNhat,soVeCaoNhat,tongGiaThapNhat,tongGiaCaoNhat,ngayDat)) {
					modelDonDat.addRow(donDatTour.toString().split(";"));
				}
				}
				else SomeStaticMethod.showDialog(JOptionPane.WARNING_MESSAGE, "Lọc không ra!");
			}
		});
		CustomButton customButton_1 = new CustomButton((String) null);
		customButton_1.setBounds(351, 488, 1, 1);
	...
	private void GUI_HuongDanVien(){
		...
		customTable.setBounds(10, 90, 761, 387);
		ArrayList<HuongDanVien> huongDanVienTim = null;
		ArrayList<HuongDanVien> dsHDV = huongDanVien_BUS.getDSHuongDanVien();
		if(doFindEvent) {
			String noiDungTimKiem = txtTim.getText().trim();
			huongDanVienTim = huongDanVien_BUS.timKiem(noiDungTimKiem);
			if (huongDanVienTim.size()==0) {
				SomeStaticMethod.showDialog(JOptionPane.WARNING_MESSAGE, "Không tìm thấy kết quả nào!");
				for (HuongDanVien huongDanVien : dsHDV) modelHuongDanVien.addRow(huongDanVien.toString().split(";"));
			}
			else {
				//customTable.xoaTable();
				for (HuongDanVien huongDanVien : huongDanVienTim) {
					modelHuongDanVien.addRow(huongDanVien.toString().split(";"));
					//System.out.println(donDatTour.toString());
				}
			}
		}else {
			for (HuongDanVien huongDanVien : dsHDV) {
			modelHuongDanVien.addRow(huongDanVien.toString().split(";"));
		}
		}
...
//		
		...
		HDV_btnThem.setIcon(new ImageIcon(ManHinhNguoiQuanLy.class.getResource("/images/them.png")));
		...
		HDV_btnXoa.setIcon(new ImageIcon(ManHinhNguoiQuanLy.class.getResource("/images/xoa.png")));
		...
		HDV_btnSua.setIcon(new ImageIcon(ManHinhNguoiQuanLy.class.getResource("/images/sua.png")));
		...
		HDV_btnXoaTrang.setIcon(new ImageIcon(ManHinhNguoiQuanLy.class.getResource("/images/pen.png")));
	...
	private String[] updateCboBoxDiemDi() {
		...
		for (Tour t : tour_BUS.getTourForManager()) {
